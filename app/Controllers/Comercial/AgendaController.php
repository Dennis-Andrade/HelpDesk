...
final class AgendaController
{
    ...
    public function export(): void
    {
        ...
        $format = strtolower((string)($_GET['formato'] ?? $_GET['format'] ?? 'vcf'));

        if ($format === 'csv') {
            $this->exportarCsv($rows);
            return;
        }
    ...
    private function exportarCsv(array $rows): void
    {
        $filename = 'agenda-' . date('Ymd') . '.csv';
        ...
        foreach ($rows as $row) {
            $entidad  = (string)($row['coop_nombre'] ?? $row['entidad'] ?? '');
            $contacto = (string)($row['oficial_nombre'] ?? $row['contacto'] ?? '');
            $cargo    = (string)($row['cargo'] ?? '');
            $telefono = (string)($row['telefono_contacto'] ?? ($row['coop_telefono'] ?? ''));
            $correo   = (string)($row['oficial_correo'] ?? ($row['coop_email'] ?? ''));
            $fecha    = (string)($row['fecha_evento'] ?? $row['created_at'] ?? '');
