...
SQL,
        ...
        if (!isset($row['segmento']) && isset($row['id_segmento'])) {
            $row['segmento'] = (string)$row['id_segmento'];
        } elseif (isset($row['id_segmento'])) {
            $row['segmento'] = (string)$row['id_segmento'];
        }
    ...
    {
        $sql = 'INSERT INTO ' . self::TABLE . ' (nombre, ruc, telefono, email, provincia_id, canton_id, id_segmento)
                VALUES (:nombre, :ruc, :telefono, :email, :provincia_id, :canton_id, :id_segmento)
        ...
        $params = array(
            ...
            ':id_segmento'  => $this->bindNullableInt($data['id_segmento'] ?? null),
        );
