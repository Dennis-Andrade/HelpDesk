...
final class EntidadRepository extends BaseRepository
{
    ...
    private const COL_LOGO          = 'logo_path';
    private const COL_CALLE         = 'direccion_calle';
    private const COL_INTERSECCION  = 'direccion_interseccion';
    private const COL_CREATED       = 'fecha_registro';
    ...
    private const T_CONTRATOS       = 'public.contrataciones_servicios';
    private const COL_CONTRATO_TOTAL = 'valor_total';
    private const COL_CONTRATO_BASE  = 'valor_contratado';
    private const COL_CONTRATO_LIC   = 'numero_licencias';
    private const COL_CONTRATO_ACTIVO = 'activo';

    private const SERVICE_MATRIX    = 1;
    private const SERVICE_CORE      = 8;
    private const SERVICE_RIESGOS   = 9;
    ...
    public function search(string $q, int $page, int $perPage): array
    {
        ...
        $sqlLines = array(
            ...
            '    NULLIF(TRIM(c.' . self::COL_LOGO . '), ' . "''" . ') AS logo_path,',
            ...
            '    c.' . self::COL_CALLE . ' AS direccion_calle,',
            '    c.' . self::COL_INTERSECCION . ' AS direccion_interseccion,',
            '    df.direccion AS direccion_facturacion,',
            '    c.' . self::COL_CREATED . ' AS fecha_registro,',
            '    fact.total_facturacion,',
            '    fact.sic_licencias',
            'FROM ' . self::T_COOP . ' c',
            ...
            'LEFT JOIN LATERAL (',
            '    SELECT',
            '        COALESCE(SUM(CASE WHEN cs.' . self::COL_CONTRATO_ACTIVO . ' = true THEN COALESCE(cs.' . self::COL_CONTRATO_TOTAL . ', cs.' . self::COL_CONTRATO_BASE . ', 0) ELSE 0 END), 0) AS total_facturacion,',
            '        COALESCE(SUM(CASE WHEN cs.' . self::PIV_SERV . ' = 3 AND cs.' . self::COL_CONTRATO_ACTIVO . ' = true THEN',
            '            COALESCE(cs.' . self::COL_CONTRATO_LIC . ', 0)',
            '            + COALESCE(cs.licencias_sic_matrix, 0)',
            '            + COALESCE(cs.licencias_sic_gratis, 0)',
            '        ELSE 0 END), 0) AS sic_licencias',
            '    FROM ' . self::T_CONTRATOS . ' cs',
            '    WHERE cs.' . self::PIV_COOP . ' = c.' . self::COL_ID,
            ') AS fact ON TRUE',
            'WHERE (',
            '    :has_q = 0',
            ...
            '    )',
            ')',
            'ORDER BY COALESCE(c.' . self::COL_CREATED . ', CURRENT_DATE) DESC, c.' . self::COL_ID . ' DESC',
    ...
    public function findById(int $id): ?array
    {
        $sql = '
            SELECT
                ...
                ' . self::COL_RUC . '        AS nit,
                NULLIF(TRIM(' . self::COL_TFIJ . '), ' . "''" . ')      AS telefono_fijo,
                NULLIF(TRIM(' . self::COL_TFIJ . '), ' . "''" . ')      AS telefono_fijo_1,
                NULLIF(TRIM(' . self::COL_TMOV . '), ' . "''" . ')      AS telefono_movil,
                ...
                ' . self::COL_LOGO . '       AS logo_path,
                ' . self::COL_CALLE . '      AS direccion_calle,
                ' . self::COL_INTERSECCION . ' AS direccion_interseccion,
                ' . self::COL_CREATED . '     AS fecha_registro,
    ...
    public function update(int $id, array $d): void
    {
        $sql = '
            UPDATE ' . self::T_COOP . ' SET
                ...
                ' . self::COL_LOGO . '     = :logo,
                ' . self::COL_CALLE . '    = :calle,
                ' . self::COL_INTERSECCION . ' = :interseccion,
    ...
    public function replaceServicios(int $id, array $ids): void
    {
        ...
        if (in_array(self::SERVICE_MATRIX, $ids, true)) {
            $filtered = array();
            foreach ($ids as $sid) {
                if ($sid === self::SERVICE_MATRIX
                    || $sid === self::SERVICE_CORE
                    || $sid === self::SERVICE_RIESGOS
                ) {
                    $filtered[] = $sid;
                }
            }
    ...
    private function hydrateListado(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $hydrated[] = array(
                ...
                'direccion_calle'  => isset($row['direccion_calle']) ? $this->trimOrNull($row['direccion_calle']) : null,
                'direccion_interseccion' => isset($row['direccion_interseccion']) ? $this->trimOrNull($row['direccion_interseccion']) : null,
                'direccion_facturacion' => isset($row['direccion_facturacion']) ? $this->trimOrNull($row['direccion_facturacion']) : null,
                'fecha_registro'   => isset($row['fecha_registro']) ? (string)$row['fecha_registro'] : null,
                'facturacion_total'=> isset($row['total_facturacion']) ? (float)$row['total_facturacion'] : 0.0,
     ...
     */
    public function listadoLigero(): array
    {
        $sql = 'SELECT ' . self::COL_ID . ' AS id, ' . self::COL_NOMBRE . ' AS nombre FROM ' . self::T_COOP . ' ORDER BY ' . self::COL_NOMBRE . ' ASC';
    ...
    private function buildEntidadParams(array $d): array
    {
        $params = array(
            ...
            ':logo'    => $this->nullableStringParam($d['logo_path'] ?? null),
            ':calle'   => $this->nullableStringParam($d['direccion_calle'] ?? $d['calle_principal'] ?? ''),
            ':interseccion' => $this->nullableStringParam($d['direccion_interseccion'] ?? $d['interseccion'] ?? ''),
