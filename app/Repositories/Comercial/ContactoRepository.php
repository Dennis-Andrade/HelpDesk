 ...
 *
 * Encapsula todas las operaciones de base de datos relacionadas con la
 * agenda de contactos. Se utilizan JOIN y comparaciones estrictas para
 * optimizar el rendimiento y mantener la coherencia con el resto del
 * proyecto.
 */
final class ContactoRepository extends BaseRepository
{
    /** @var string Nombre de la tabla de contactos. */
    private const T_CONTACTO      = 'public.contactos';
    /** @var string Clave primaria del contacto. */
    private const COL_ID          = 'id_contacto';
    /** @var string Columna de relación con la cooperativa. */
    private const COL_COOP        = 'id_cooperativa';
    /** @var string Columna de nombre del contacto. */
     ...
     *
     * Permite filtrar por nombre de contacto. La consulta devuelve el total
     * de registros y una lista de resultados acorde a la paginación.
     *
    ...
    public function search(string $q, int $page, int $perPage): array
    {
        ...
        $countSql = '
            ...
            WHERE (
                :has_q = 0
                OR unaccent(lower(c.' . self::COL_NOMBRE . ')) LIKE unaccent(lower(:like))
            )
        ';
        $bindings = [
    ...
    public function create(array $d): int
    {
        ...
        ';
        $params = [
            ':id_cooperativa' => [$d['id_cooperativa'], PDO::PARAM_INT],
            ':nombre'         => $this->nullableStringParam($d['nombre'] ?? ''),
            ':titulo'         => $this->nullableStringParam($d['titulo'] ?? ''),
            ':cargo'          => $this->nullableStringParam($d['cargo'] ?? ''),
            ':telefono'       => $this->nullableStringParam($d['telefono_contacto'] ?? ''),
            ':correo'         => $this->nullableStringParam($d['email_contacto'] ?? ''),
     ...
     *
     * Convierte cadenas vacías en NULL y prepara un array con valor y tipo.
     *
     * @param mixed $value
