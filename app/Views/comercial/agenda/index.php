...
$exportQuery = http_build_query($exportParams, '', '&', PHP_QUERY_RFC3986);
$exportVcfUrl = '/comercial/agenda/exportar' . ($exportQuery !== '' ? '?' . $exportQuery : '');
$exportCsvUrl = '/comercial/agenda/exportar' . ($exportQuery !== '' ? '?' . $exportQuery . '&formato=csv' : '?formato=csv');
...
<section class="agenda-page">
  <header class="agenda-page__header">
    ...
    <div class="agenda-page__exports">
      <a class="agenda-page__export" href="<?= agenda_h($exportVcfUrl) ?>">
        <span class="material-icons">contact_mail</span> Descargar VCF
      </a>
      <a class="agenda-page__export" href="<?= agenda_h($exportCsvUrl) ?>">
        <span class="material-icons">table_rows</span> Descargar CSV
      </a>
    </div>
  </header>
    ...
    <form method="get" class="agenda-filters" action="/comercial/agenda">
      <label>
        ...
        <input
          ...
          name="q"
          value="<?= $filterText ?>"
          placeholder="Entidad, nombre o cargo"
          autocomplete="off"
          spellcheck="false"
          data-typeahead="generic"
          data-suggest-url="/comercial/contactos/sugerencias"
          data-suggest-min="3"
          data-suggest-value="term"
          data-suggest-label="label"
          list="agenda-search-suggestions">
        <datalist id="agenda-search-suggestions"></datalist>
      </label>
      <label>
        Entidad
        <select name="coop">
