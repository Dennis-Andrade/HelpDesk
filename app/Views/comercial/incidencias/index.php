...
<section class="ent-list ent-incidencias" aria-labelledby="incidencias-title">
  <header class="ent-toolbar">
    <div class="ent-toolbar__lead">
      <h1 id="incidencias-title" class="ent-title">Gestiones comerciales</h1>
      <p class="ent-toolbar__caption" aria-live="polite">
        <?= h((string)$total) ?> gestiones · Página <?= h((string)$page) ?> de <?= h((string)$pages) ?>
      </p>
    </div>
  </header>
  ...
  <section class="incidencias-card incidencias-card--list">
      <form class="incidencias-filters" method="get" action="/comercial/incidencias" role="search">
        <div class="incidencias-filters__field">
          ...
          <input
            id="incidencias-ticket"
            type="text"
            name="ticket"
            value="<?= h($ticketFiltro) ?>"
            placeholder="Ej. INC-2024-0001"
            autocomplete="off"
            spellcheck="false"
            data-typeahead="generic"
            data-suggest-url="/comercial/eventos/tickets/buscar"
            data-suggest-min="2"
            data-suggest-value="codigo"
            data-suggest-label="titulo"
            data-suggest-merge="true"
            list="incidencias-ticket-suggestions">
          <datalist id="incidencias-ticket-suggestions"></datalist>
        </div>
        <div class="incidencias-filters__field">
        ...
        <div class="incidencias-filters__field">
          <label for="incidencias-coop">Entidad</label>
      ...
      <?php if (empty($items)): ?>
        <div class="card incidencias-empty" role="status">No hay gestiones registradas con los filtros seleccionados.</div>
      <?php else: ?>
        <div class="incidencias-cards" role="list" aria-label="Listado de gestiones">
            ...
            ?>
              <article class="incidencias-card-item incidencias-row" role="listitem"
                ...
                <div class="incidencias-card-item__grid">
                  <div>
                    <span class="incidencias-card-item__label">Entidad</span>
                    <p><?= h($coopNombre) ?></p>
                  </div>
                  <div>
                  ...
                  <div>
                  <span class="incidencias-card-item__label">Tipo de gestión</span>
                    <p><?= h($tipo !== '' ? $tipo : '—') ?></p>
                  </div>
                </div>
                ...
                <footer class="incidencias-card-item__footer">
                  ...
                  <div class="incidencias-card-item__actions">
                    ...
                    <form method="post" action="/comercial/incidencias/<?= h((string)$id) ?>/eliminar" class="incidencias-delete" onsubmit="return confirm('¿Deseas eliminar esta gestión?');">
...
<div class="incidencias-modal" id="incidencias-create-modal" role="dialog" aria-modal="true" aria-hidden="true" tabindex="-1" hidden>
  ...
  <div class="incidencias-modal__card" role="document">
    <header class="incidencias-modal__header">
      <h2><span class="material-symbols-outlined" aria-hidden="true">note_add</span> Registrar gestión</h2>
    ...
    <form class="incidencias-modal__form" id="incidencias-create-form" method="post" action="/comercial/incidencias" autocomplete="off">
      <div class="incidencias-modal__grid">
        <div class="incidencias-modal__field">
          <label for="create-cooperativa-search">Entidad</label>
          <input
            id="create-cooperativa-search"
            type="text"
            placeholder="Escribe el nombre o RUC"
            autocomplete="off"
            spellcheck="false"
            data-incidencia-entity-search>
          <select
            id="create-cooperativa"
            name="id_cooperativa"
            required
            data-incidencia-entity-select
            style="display:none;"
            aria-hidden="true">
          ...
          <small class="incidencias-modal__hint" data-incidencia-entity-hint hidden>
            Si no encuentras la entidad, regístrala desde Comercial &gt; Entidades antes de continuar.
          </small>
        </div>
        <div class="incidencias-modal__field">
        ...
        <div class="incidencias-modal__field">
          ...
          <input id="create-asunto" type="text" name="asunto" required maxlength="180" placeholder="Resumen de la gestión">
        </div>
        <div class="incidencias-modal__field">
          <label for="create-tipo">Tipo de gestión</label>
...
<div class="incidencias-modal" id="incidencias-modal" role="dialog" aria-modal="true" aria-hidden="true" tabindex="-1" hidden>
  ...
  <div class="incidencias-modal__card" role="document">
    <header class="incidencias-modal__header">
      <h2><span class="material-symbols-outlined" aria-hidden="true">visibility</span> Detalle de la gestión</h2>
      <button type="button" class="incidencias-modal__close" aria-label="Cerrar" data-incidencia-close>&times;</button>
    </header>
    <form class="incidencias-modal__form" id="incidencias-modal-form" method="post" action="">
      <div class="incidencias-modal__grid">
        ...
        <div class="incidencias-modal__field">
          <label for="modal-cooperativa">Entidad</label>
    ...
    <section class="incidencias-modal__contacto" aria-labelledby="contacto-titulo">
      <h3 id="contacto-titulo">Contacto de la entidad</h3>
