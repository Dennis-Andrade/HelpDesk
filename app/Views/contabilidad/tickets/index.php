...
<section class="ent-container" aria-labelledby="tickets-contables-title">
  <header class="ent-toolbar">
    <div class="ent-toolbar__lead">
      <h1 id="tickets-contables-title" class="ent-title">Tickets contables</h1>
      <p class="ent-toolbar__caption">
        <?= ctk_list_h((string)$total) ?> tickets · Página <?= ctk_list_h((string)$page) ?> de <?= ctk_list_h((string)max(1, $pages)) ?>
      </p>
    </div>
  ...
  <div class="seguimiento-card seguimiento-card--filters contab-tickets-filters">
    <form class="seguimiento-filters" method="get" action="/contabilidad/tickets" role="search">
      ...
      <div class="seguimiento-filters__field seguimiento-filters__field--wide">
        ...
        <input id="tickets-q" type="text" name="q" value="<?= ctk_list_h((string)($filters['q'] ?? '')) ?>" placeholder="Código, asunto o descripción">
      </div>
  ...
  <?php if (!$items): ?>
    <div class="seguimiento-empty">
      ...
      <p>No hay tickets contables registrados.</p>
    </div>
  <?php else: ?>
          ...
          <?php foreach ($items as $row): ?>
            <tr>
              ...
              <td class="table-actions">
                ...
                <form method="post" action="/contabilidad/tickets/eliminar" onsubmit="return confirm('¿Eliminar este ticket?');">
