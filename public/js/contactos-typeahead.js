...
(function () {
  ...
  function scheduleHide(toast, delay) {
  ...
  function showToast(message, variant) {
    ...
    scheduleHide(toast, 4200);
  }

  function autoDismissInitial() {
    var initial = document.getElementById('ent-toast');
    if (initial && initial.textContent && initial.textContent.trim() !== '') {
      scheduleHide(initial, 5200);
    }
  }
  ...
  function validateContactForm(form) {
    ...
    var entidad = form.querySelector('select[name="id_entidad"]');
    var nombre = form.querySelector('input[name="nombre"]');
    var telefonoEl = form.querySelector('input[name="telefono"]');
    var correoEl = form.querySelector('input[name="correo"]');

    if (entidad && entidad.value.trim() === '') {
      showToast('Selecciona la entidad del contacto.', 'error');
      if (entidad.focus) { entidad.focus(); }
      return false;
    }

    if (!nombre || nombre.value.trim() === '') {
      showToast('Ingresa el nombre del contacto.', 'error');
      if (nombre && nombre.focus) { nombre.focus(); }
      return false;
    }

    var telefono = telefonoEl ? sanitizePhone(telefonoEl.value) : '';
    var correo = correoEl ? correoEl.value.trim() : '';

    if (telefono === '' && correo === '') {
      showToast('Registra al menos un celular o correo.', 'error');
      if (telefonoEl && telefonoEl.focus) { telefonoEl.focus(); }
      return false;
    }

    if (telefono !== '' && (telefono.length < 7 || telefono.length > 10)) {
      showToast('El celular debe tener entre 7 y 10 dígitos.', 'error');
      if (telefonoEl && telefonoEl.focus) { telefonoEl.focus(); }
      return false;
    }
    ...
    if (correo !== '' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(correo)) {
      ...
      if (correoEl && correoEl.focus) { correoEl.focus(); }
      return false;
    }
  ...
  document.addEventListener('DOMContentLoaded', autoDismissInitial);

  document.querySelectorAll('.contact-modal__form').forEach(function (form) {
